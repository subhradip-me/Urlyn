...
const switchPersona = asyncHandler(async (req, res) => {
  ...
  console.log('🎭 Persona switch request:', { persona, userId: req.user?._id, isDevelopment: process.env.NODE_ENV === 'development' });
  ...
  // Special handling for development mode with mock users
  if (process.env.NODE_ENV === 'development' && typeof req.user._id === 'string' && req.user._id.startsWith('68d0e521')) {
    console.log('🧪 Development mode persona switch for mock user');
    // For mock users, just return success without database operations
    res.json({
      message: `Switched to ${persona} persona (development mode)`,
